 ...
 * @details
 * - Test Steps
 *   -# Set dma configuration with in memory to memory direction with a single block transfer
 *   -# TODO: Complete the test steps
 */
...
static __aligned(32) uint8_t rx_data[CONFIG_DMA_BA_XFER_SIZE] = {0};
...
static void dma_ba_callback_restart_transfer(const struct device *dma_dev, void *user_data,
...
static int test_ba_restart_transfer(void)
{
	...
	dma_cfg.dma_callback = dma_ba_callback_restart_transfer;
	...
	dma_block_cfgs.block_size = CONFIG_DMA_BA_XFER_SIZE/2;
	...
	TC_PRINT("block_size %d, source addr %x, dest addr %x\n",
		 CONFIG_DMA_BA_XFER_SIZE, dma_block_cfgs.source_address,
		 dma_block_cfgs.dest_address);
	...
	dma_block_cfgs.source_address = (uint32_t)(tx_data) + CONFIG_DMA_BA_XFER_SIZE/2;
	dma_block_cfgs.dest_address = (uint32_t)(rx_data) + CONFIG_DMA_BA_XFER_SIZE/2;
	
	/* Be sure that the DMA transfer is done */
	k_busy_wait(1*1000); /* busy wait */
	...
	TC_PRINT("Finished: DMA block append busy wait\n");
	return TC_PASS;
}

static void dma_ba_callback_restart_in_isr(const struct device *dma_dev, void *user_data,
			    uint32_t channel, int status)
{
	if (status < 0) {
		TC_PRINT("callback status %d\n", status);
...
static int test_ba_restart_in_isr(void)
{
	...
	dma_cfg.dest_burst_length = 1U;
	dma_cfg.user_data = NULL;
	dma_cfg.dma_callback = dma_ba_callback_restart_in_isr;
	dma_cfg.head_block = &dma_block_cfgs;
	dma_cfg.complete_callback_en = true; /* per block completion */
	...
	/* Remove the done flag of the dma channel to simulate an append while a transfer is ongoing*/
	...
	/* Set the chdone bit to simulate that the DMA transfer was finished while appending a block*/
	sys_set_bit((mem_addr_t)&LDMA->CHDONE, chan_id);

	/* Check if the isr in dma driver will correctly restart the DMA engine with the new block that was append*/
	irq_unlock(key);
...
static void dma_ba_callback_loopstress_in_isr(const struct device *dma_dev, void *user_data,
			    uint32_t channel, int status)
{
	...
	if (status < 0) {
		...
		if(rx_idx <= CONFIG_DMA_BA_XFER_SIZE - BLOCK_SIZE){
			dma_block_cfgs.source_address = (uint32_t)(tx_data + tx_idx);
			dma_block_cfgs.dest_address = (uint32_t)(rx_data + rx_idx);
			rx_idx += BLOCK_SIZE;
			tx_idx += BLOCK_SIZE;
...
static int test_ba_loopstress_in_isr(void)
{
	const struct device *dma;
	static int chan_id;
	unsigned int key;
	...
	dma_cfg.dma_callback = dma_ba_callback_loopstress_in_isr;
	dma_cfg.head_block = &dma_block_cfgs;
	dma_cfg.complete_callback_en = true; /* per block completion */
	...
	dma_block_cfgs.block_size = BLOCK_SIZE;
	dma_block_cfgs.source_address = (uint32_t)(tx_data + tx_idx);
	dma_block_cfgs.dest_address = (uint32_t)(rx_data + rx_idx);
	rx_idx += BLOCK_SIZE;
	tx_idx += BLOCK_SIZE;
	...
	TC_PRINT("dma block %d block_size %d, source addr %x, dest addr %x\n",
		  0, BLOCK_SIZE, dma_block_cfgs.source_address,
		  dma_block_cfgs.dest_address);
...
static int test_ba_loopstress(void)
{
	...
	if (chan_id < 0) {
		TC_PRINT("Platform does not support dma request channel,"
			 " using Kconfig DMA_BA_CHANNEL_NR\n");
		chan_id = CONFIG_DMA_BA_CHANNEL_NR;
	}
...
ZTEST(dma_m2m_ba, test_dma_m2m_loopstress_in_isr)
{
	zassert_true((test_ba_loopstress_in_isr() == TC_PASS));
}
