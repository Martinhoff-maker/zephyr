					  ...
					  struct dma_block_config *block, LDMA_Descriptor_t *desc)
{
	...
	if (block->block_size % config->source_data_size) {
		desc->xfer.xferCnt = block->block_size / config->source_data_size;
	} else {
		desc->xfer.xferCnt = block->block_size / config->source_data_size - 1;
	}
					   ...
					   struct dma_silabs_channel *chan_conf)
{
	...
	while (block) {
		...
		if (block == head_block) {
			block = NULL;
			prev_desc->xfer.linkAddr = LDMA_DESCRIPTOR_LINKABS_ADDR_TO_LINKADDR(chan_conf->desc);
			prev_desc->xfer.linkMode = ldmaLinkModeAbs;
			prev_desc->xfer.link = 1;
		}
	}
